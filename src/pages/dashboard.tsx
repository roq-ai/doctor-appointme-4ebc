...
import { getPatientRecords } from 'apiSdk/patient-records';
import { PatientRecordInterface } from 'interfaces/patient-record';
import { getSchedules } from 'apiSdk/schedules';
import { ScheduleInterface } from 'interfaces/schedule';

import { ScheduleListPage as ShowCaseList } from 'pages/schedules';

function DashboardPage() {
  const { session } = useSession();

  const appointmentStat = useSWR<PaginatedInterface<AppointmentInterface>>(
    () => `/appointments`,
    () => getAppointments({ limit: 1, offset: 0 }),
  );

  const patientRecordStat = useSWR<PaginatedInterface<PatientRecordInterface>>(
    () => `/patient-records`,
    () => getPatientRecords({ limit: 1, offset: 0 }),
  );

  const scheduleStat = useSWR<PaginatedInterface<ScheduleInterface>>(
    () => `/schedules`,
    () => getSchedules({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Appointments',
      result: appointmentStat?.data?.totalCount,
      isLoading: appointmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Patient Records',
      result: patientRecordStat?.data?.totalCount,
      isLoading: patientRecordStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Schedules',
      result: scheduleStat?.data?.totalCount,
      isLoading: scheduleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`schedules`} passHref legacyBehavior>
                  <Button
